...
my $passwords = get_input();
say solve_part_1($passwords);
say solve_part_2($passwords);

sub solve_part_1($passwords) {
    scalar grep no_repeat_words($_), @$passwords;
}

sub solve_part_2($passwords) {
    scalar grep no_anagrams($_), @$passwords;
}

sub no_anagrams($password) {
    my @words = split ' ', $password, -1;
    my %seen;
    for my $word (@words) {
        my $w = join '', sort split //, $word;
        return 0 if $seen{$w}++;
    }
    return 1;
}

sub no_repeat_words($password) {
    my %seen;
    for my $word (split ' ', $password, -1) {
        return 0 if $seen{$word}++;
    }
    return 1;
}
