...
my $table = get_input();
say solve_part1($table);
say solve_part2($table);

sub solve_part1($table) {
    my $sum = 0;
    for my $row (@$table) {
        my ($min, $max) = min_max($row);
        $sum += $max - $min;
    }
    $sum;
}

sub solve_part2($table) {
    my $sum = 0;
    for my $row (@$table) {
        my ($a, $b) = divisible_pair($row);
        $sum += $a / $b;
    }
    $sum;
}

sub divisible_pair($nums) {
    croak 'nums is too small' unless @$nums >= 2;
    for my $i (0..$#$nums-1) {
        for my $k ($i+1..$#$nums) {
            my ($a, $b) = @$nums[$i, $k];
            ($a, $b) = ($b, $a) if $a < $b;
    ...
    croak 'divisible pair not found';
}

sub min_max($nums) {
    croak 'nums is empty' unless @$nums;
    my ($min, $max) = @$nums[0, 0];
    for my $num (@$nums[1..$#$nums]) {
        $min = $num if $num < $min;
        $max = $num if $num > $max;
    }
    ($min, $max);
