...
my $records = get_input();
say 'part 1: ', solve_part_1($records);
say 'part 2: ', solve_part_2($records);

sub solve_part_1($records) {
    my $tree = make_tree($records);
    (keys %$tree)[0];
}

sub solve_part_2($records) {
    my $tree = make_tree($records);
    compute_weights($tree);
    correct_unballanced_weight($tree);
}

sub compute_weights($tree) {
    my $sum = 0;
    for my ($key, $node) (%$tree) {
        $sum += $node->{total_weight} = sum(
            $node->{weight},
            compute_weights($node->{children}),
        );
    }
    $sum;
}

sub correct_unballanced_weight($tree) {
    my $correct_weight;
    find_incorrect_weight($tree, sub($cw) {
        # вызовется несколько раз, последний раз для наиболее вложенного
        $correct_weight = $cw;
    });
    $correct_weight;
}

sub find_incorrect_weight($tree, $cb) {
    for my ($key, $node) (%$tree) {
        my $corrected_weight = corrected_weight($node->{children});
        if (defined $corrected_weight) {
            $cb->($corrected_weight);
        }
        find_incorrect_weight($node->{children}, $cb);
    }
}

sub corrected_weight($nodes) {
    my %weights_cnt;
    my $correct_total_weight;
    for my ($node_name, $node) (%$nodes) {
        $weights_cnt{$node->{total_weight}}++;
        if ($weights_cnt{$node->{total_weight}} >= 2) {
            $correct_total_weight = $node->{total_weight};
        }
    }
    return undef if %weights_cnt == 1;
    for my ($node_name, $node) (%$nodes) {
        if ($weights_cnt{$node->{total_weight}} == 1) {
            return $node->{weight} - ($node->{total_weight} - $correct_total_weight);
        }
    }
    return undef;
}

sub make_tree($records) {
    my %nodes;
    for my $r (@$records) {
        $nodes{$r->[0]} = {
            weight   => $r->[1],
    ...
    my %tree;
    while (%nodes) {
        my $node_name = each %nodes;
        keys %nodes;
        _make_tree(\%tree, \%tree, \%nodes, $node_name);
    }

    my @nodes = keys %tree;
    die "tree has " . @nodes . " nodes" unless @nodes == 1;

    \%tree;
}

sub _make_tree($top_tree, $tree, $nodes, $node_name) {
    my $node = delete $nodes->{$node_name};
    $tree->{$node_name} = {
        weight   => $node->{weight},
        children => {},
    };
    for my $child ($node->{children}->@*) {
        if (my $subtree = delete $top_tree->{$child}) {
            $tree->{$node_name}{children}{$child} = $subtree;
            next;
        }
        _make_tree($top_tree, $tree->{$node_name}{children}, $nodes, $child);
    }
}
...
sub get_input() {
    my @records;
    while (<DATA>) {
        /^(\w+) \((\d+)\)(?: -> (.+))?$/ or die "failed to parse '$_'";
        push @records, [ $1, $2, $3 ? [ split /, /, $3, -1 ] : [] ];
    }
    close DATA;
    \@records;
}
